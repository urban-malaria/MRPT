...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        // Custom Analysis Confirmation
        else if (e.target.id === 'confirm-custom-analysis') {
            e.preventDefault();
            const paramsContainer = e.target.closest('.custom-analysis-confirmation');
            if (paramsContainer) paramsContainer.innerHTML = "<strong>Running custom analysis...</strong>";
            const paramsStr = e.target.getAttribute('data-params');
            if (paramsStr) {
                try {
                    const customParams = JSON.parse(paramsStr);
                    runAnalysisWithVariables(customParams.variables);
                } catch (err) {
                    console.error('Error parsing custom params:', err);
                    addSystemMessage("<strong>Error parsing custom analysis parameters.</strong>");
                }
            } else {
                 addSystemMessage("<strong>Could not find custom analysis parameters.</strong>");
            }
        } else if (e.target.id === 'cancel-custom-analysis') {
            e.preventDefault();
            const paramsContainer = e.target.closest('.custom-analysis-confirmation');
            if(paramsContainer) paramsContainer.innerHTML = "<strong>Custom analysis cancelled.</strong>";
        }
    ...
    function sendMessage() {
        ...
        // --- Existing logic (visualization, custom analysis, run analysis, general message) ---
        if (isVisualizationRequest(message)) {
            handleVisualizationRequest(message); // Pass original message
            return;
        }

        const customVariables = parseCustomAnalysisRequest(message);
        if (customVariables && customVariables.length > 0) {
            showCustomAnalysisConfirmation(customVariables);
             // Don't return here, let the confirmation buttons handle the next step
            hideTypingIndicator(); // Confirmation is the response
    ...
    function addVisualization(vizPath, title, vizType, vizData = {}) {
        ...
        messageDiv.className = 'message assistant-message new-message'; // Add new-message for animation
        ...
        if (vizPath.endsWith('.html')) {
            ...
            iframe.width = '100%';
            iframe.height = (vizType === 'composite_map' || vizType === 'vulnerability_plot') ? '550px' : '450px';
    ...
    function parseCustomAnalysisRequest(message) {
        const lowerMsg = message.toLowerCase();
        if (isCustomVariableRequest(lowerMsg)) {
            return extractVariables(message); // Use the function below
        }
        return null;
    }

    function isCustomVariableRequest(lowerMsg) {
        const customPatterns = [
            /run.+(?:with|using|for|only|these).+variables?/i,
            /analyze.+(?:with|using|for|only|these).+variables?/i,
            /use.+(?:variable|variables)/i,
            /custom.+(?:analysis|variables)/i,
            /rerun.+(?:with|using)/i,
            /only.+(?:use|include|consider).+variables?/i,
            /following\s+variables/i,
            /variables?:?\s+([\w\s_,]+)/i // Look for "variables: x, y, z"
        ];
        return customPatterns.some(pattern => pattern.test(lowerMsg));
    }

    function extractVariables(message) {
        const lowerMsg = message.toLowerCase();
        let extractedVars = new Set(); // Use a Set to avoid duplicates

        // Patterns to capture lists after keywords like "variables:", "using:", "with:"
        const listPatterns = [
            /\b(?:variables?|using|with|include|consider|only)\s*:?\s+((?:[\w_]+(?:,\s*|\s+and\s+|\s+))+[\w_]+)/gi, // Comma, 'and', or space separated after keyword
            /\b(?:variables?|using|with|include|consider|only)\s*:?\s+((?:[\w_]+,?)+)/gi // Primarily comma separated or single word
        ];

        for (const pattern of listPatterns) {
             // Use matchAll to find all occurrences
             for (const match of lowerMsg.matchAll(pattern)) {
                if (match[1]) {
                    const variableText = match[1].trim();
                    // Split by comma, 'and', or spaces, then clean up
                    variableText.split(/(?:,\s*|\s+and\s+|\s+)/)
                        .map(v => v.trim().replace(/[.,;:?!]$/, '')) // Remove trailing punctuation
                        .filter(v => v && v.length > 2 && !/^(?:the|a|an|variable|variables|and|for|with|using|only|just|these|following|list)$/.test(v)) // Basic stop word filter
                        .forEach(v => extractedVars.add(v.replace(/\s+/g, '_'))); // Normalize spaces
                }
            }
        }


        // If no list pattern matched well, try finding known variables mentioned anywhere
        if (extractedVars.size === 0) {
            const commonVariables = [
                ...
                'urbanPercentage', 'building_height', 'pfpr', 'rainfall', 'temperature', 'elevation',
                'population', 'ndvi', 'evi', 'ndwi', 'flood'
                // Add more known variable stems if needed
            ];

            for (const variable of commonVariables) {
                 const regex = new RegExp(`\\b${variable.replace('_', '[_\\s]?')}\\b`, 'i'); // Allow space or underscore
                 if (regex.test(lowerMsg)) {
        ...
        console.log("Extracted variables:", Array.from(extractedVars));
        return Array.from(extractedVars); // Convert Set back to Array
    }
    ...
    function showCustomAnalysisConfirmation(variables) {
        ...
        let detailsMessage = "Okay, I can run the analysis using the variables you mentioned. Please confirm the list below:<br>";
        let confirmButtonHtml = '';

        if (variables && variables.length > 0) {
            detailsMessage += `<ul style="margin-top: 5px;">${variables.map(v => `<li>${v}</li>`).join('')}</ul>`;
            // Only show confirm button if variables were extracted
            const paramsStr = JSON.stringify({ variables: variables });
            confirmButtonHtml = `<button class="btn btn-primary btn-sm mt-2" id="confirm-custom-analysis" data-params='${escapeHTML(paramsStr)}'>Confirm and Run</button>`; // Escape data-params
        } else {
        ...
        addSystemMessage(`
            <div class="custom-analysis-confirmation">
                ...
                ${confirmButtonHtml}
                <button class="btn btn-secondary btn-sm mt-2 ms-2" id="cancel-custom-analysis">Cancel</button>
            </div>
        `);
    ...
    function runAnalysisWithVariables(variables) {
        if (!variables || variables.length === 0) {
             addSystemMessage("<strong>Cannot run custom analysis:</strong> No variables were provided.");
             return;
        }
        addSystemMessage("<strong>Running custom analysis with specified variables...</strong> This may take a moment.");
        ...
        fetch('/run_analysis', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ selected_variables: variables })
        })
        .then(response => response.json())
        .then(data => {
            ...
            if (data.status === 'success') {
                ...
                addSystemMessage(`<strong>Custom analysis complete!</strong><br>Used ${varsUsed.length} variables.<br>Top vulnerable: ${topWards.slice(0, 3).join(', ')}${topWards.length > 3 ? ', ...' : ''}`);
                addAssistantMessage(analysisSuccessResponseMessage(data, true)); // Pass true for custom
            } else {
                addSystemMessage(`<strong>Error running custom analysis</strong><br>${data.message || 'Unknown error'}`);
                if(data.available_variables) {
                    addSystemMessage(`Available variables include: ${data.available_variables.slice(0, 10).join(', ')}${data.available_variables.length > 10 ? '...' : ''}`);
                }
            }
            scrollToBottom();
        })
        .catch(error => {
            ...
            hideLoadingIndicator(); hideTypingIndicator();
            isWaitingForResponse = false; // Reset waiting state
            addSystemMessage("<strong>Error running custom analysis</strong><br>Could not connect to the server. Please try again.");
        });
    }
    ...
    function handleAction(data) {
        ...
        switch(data.action) {
            ...
            case 'show_report':
                ...
                break;
             case 'confirm_custom_variables':
                // Backend sent a message asking for confirmation, which is displayed.
                // The actual analysis run is handled by the user clicking the confirm button.
                // So, no specific action needed here, just ensure the message displays.
                break;
    ...
    function updateSessionStatus() {
        ...
        let statusClass = "status-display"; // Default

        if (sessionData.analysisComplete) {
            statusText = "Analysis Complete";
            statusClass = "status-display success-status";
        } else if (sessionData.csvLoaded && sessionData.shapefileLoaded) {
            statusText = "Data Loaded";
            statusClass = "status-display info-status";
        } else if (sessionData.csvLoaded || sessionData.shapefileLoaded) {
            statusText = sessionData.csvLoaded ? "CSV Loaded" : "Shapefile Loaded";
            statusClass = "status-display pending-status";
        }

        sessionStatus.textContent = statusText;
        sessionStatus.className = statusClass; // Apply the class
    }
    ...
    // Original version (before marked.js integration)
