...
## Raw data cleaning, visualization and smoothing module 

### Upload data/input variables 
...
![Missing data vs smoothed data map](images/Artboard%201-01.png)
...
## **Manual Labeling/Micro-planning** 

The **manual labeling** feature provides street-level detail, satellite imagery, and CartoDBlight basemaps for individual wards. The figure below illustrates the manual labeling process for a ward named **Ekurede**. First, users select the desired ward and define the size of the grid—these are arbitrary boundaries used to segment the ward for easier classification. For each grid cell, users can zoom in and assess the dominant features visible in the imagery. Based on these observations, the most appropriate label is assigned: **formal**, **informal**, or **no buildings/avoid area**, as shown in the example.

![](images/tab_manual_labell.png)

A closer look at the left panel is shown below. It features two dropdown menus and several tabs that allow you to select the ward of interest, define the tile size for labeling, and generate the tiled area of the selected ward. Once these steps are completed, you proceed to the labeling stage, as illustrated in the previous figure. This tab also provides options to clear your annotations, download the tiled (gridded) shapefile, and download the annotated shapefile.

![](images/closer_look-02.png)

After that you will get a completed map that has the ward of interest labelled, in case Ekurede, characterized by the dominant features in that ward as shown below.

![](images/Manual_label.png)
...
## **Implementation dashboard/Net distribution** 

The Net Distribution Planning module within the Malaria Reprioritization Tool (MRPT) is designed to support data-driven allocation of bed nets across urban settings. Leveraging settlement classifications, population data, and ward-level vulnerability rankings, the tool enables users to develop efficient, equitable, and transparent distribution plans.

This module simplifies the planning process by requiring only two key inputs - the total number of nets available and the average household size in the ward. It then uses a structured two-phase strategy to allocate nets: prioritizing the most vulnerable wards first, and redistributing any remaining nets to other areas based on defined thresholds and special considerations.

By integrating advanced features such as dynamic mapping, population coverage summaries, and downloadable output files, the MRPT provides actionable intelligence to support malaria control programs, especially in resource-constrained or crisis-prone environments.

**Instructions for Using the Net Distribution Planning Module in MRPT**

**Step 1: Input Basic Distribution Information**

1.   Navigate to the Net Distribution tab at the top menu.

2.   Under Distribution Settings:

    -   Enter the total number of bed nets available.

    -   Specify the average household size for the target area.

    *(Standard assumption: 1 net per 2 people, with at least 1 net per household.)*

**Step 2: Understand the Distribution Strategy**

The tool uses a Two-Phase Distribution Approach:

-   Phase 1: Prioritized Wards - Wards below the defined urban extent threshold (e.g., \<30% urban) are prioritized. Nets are allocated to these wards based on vulnerability rankings.

-   Phase 2: Re-prioritized Wards If nets remain after Phase 1, they are allocated to wards above the threshold that have been manually flagged for special consideration.

*A map and summary table will show you how nets are distributed across wards and highlight coverage levels (see below).*

![](images/dashboard_c.png)

Upclose visual of the dashboard

![](images/dashbord_b.png)

The related table showing the ward coverages.

![](images/dashboard_a.png)

**Step 3: View Population Data & Coverage Stats**

-   The Population Coverage section displays:

    -   Total target population

    -   Households covered

    -   Nets distributed vs. needed

    -   Coverage percentage (e.g., 3.1%)

-   It will also show how many additional nets are required to achieve full coverage.

**Step 4: Review Ward-Level Allocation**

-    Click on the Ward Coverage tab to view:
    ...
    -   Population, nets needed, nets allocated, and resulting coverage %

-   A color-coded table helps identify areas with the lowest coverage.

**Step 5: Analyze the Distribution Map**

-   The Net Distribution Map displays:

    -   Which wards received nets

    -   Prioritized vs. re-prioritized vs. de-prioritized wards

    -   Coverage level legend (ranging from full to low coverage)

**Step 6: Download Actionable Outputs**

After reviewing the plan:

-    Click “Calculate Distribution” to run the net allocation algorithm.

Then click “Download Distribution Plan” to export:

-   Net allocation tables

-   Classification maps

-   Detailed ward-level statistics

These outputs can be directly used by program managers to guide and implement bed net distribution campaigns.
...
Other Formats - Download the User Manual ([PDF](https://github.com/urban-malaria/MRMT/blob/main/docs/index.pdf)). For additional support or questions, contact: [lmhlanga\@luc.edu](mailto:lmhlanga@luc.edu) or [bboateng1\@luc.edu](mailto:bboateng1@luc.edu) (co -developers) and [**iozodiegwu\@luc.edu**](mailto:iozodiegwu@luc.edu)**(Principal Investigator**).
